...
export default function PlaylistDrawer({ isOpen, onClose, currentTrackIndex, onSelectTrack, onCustomYoutubeUrl, isPlaying, onTogglePlay }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="relative w-full sm:max-w-2xl h-[92vh] sm:h-auto sm:max-h-[85vh] glass-panel rounded-t-3xl sm:rounded-3xl p-4 sm:p-6 flex flex-col overflow-hidden shadow-2xl border border-white/20">
        
        {/* Header */}
        <div className="flex items-center justify-between pb-3 sm:pb-4 border-b border-white/10">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="p-2 sm:p-2.5 rounded-xl sm:rounded-2xl bg-amber-500/20 text-amber-400 border border-amber-500/30">
              <Music className="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white tracking-wide">Durga Puja Playlist</h2>
              <p className="text-[10px] sm:text-xs text-white/60">Songs to serenade your pandal hopping</p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 mb-3 sm:mb-4">
          <input 
            ...
            className="w-full px-3 sm:px-4 py-2 rounded-xl bg-white/5 border border-white/15 text-xs sm:text-sm text-white placeholder-white/40 focus:outline-none focus:border-amber-400/60"
          />
        </div>

        {/* Custom YouTube Link Submission Form */}
        <form onSubmit={handleAddCustom} className="flex gap-2 mb-3 sm:mb-4">
          <input 
            type="text" 
            placeholder="Paste YouTube or Spotify URL..."
            value={customInput}
            onChange={(e) => setCustomInput(e.target.value)}
            className="flex-1 px-3 sm:px-4 py-2 rounded-xl bg-white/5 border border-white/15 text-[11px] sm:text-xs text-white placeholder-white/40 focus:outline-none focus:border-amber-400/60"
          />
          <button 
            type="submit"
            className="px-2.5 sm:px-3 py-2 rounded-xl bg-amber-500 text-slate-950 text-[11px] sm:text-xs font-semibold hover:bg-amber-400 flex items-center gap-1 transition active:scale-95 shrink-0"
          >
          ...
          {filteredPlaylist.map((track, idx) => {
            ...
            return (
              <button 
                ...
                className={`group flex items-center gap-2.5 sm:gap-3.5 p-2 sm:p-2.5 rounded-xl sm:rounded-2xl cursor-pointer transition-all text-left w-full ${
                  isCurrent 
                    ? 'bg-amber-500/20 border border-amber-400/50 shadow-md' 
                    : 'bg-white/5 border border-white/10 hover:bg-white/10'
                }`}
              >
                <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl overflow-hidden shrink-0 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-xs sm:text-sm font-semibold truncate ${isCurrent ? 'text-amber-300' : 'text-white'}`}>
                    {track.title}
                  </p>
                ...
                <button 
                  ...
                  className={`p-2 rounded-full shrink-0 transition ${
                    isActivelyPlaying ? 'bg-amber-400 text-slate-950' : 'bg-white/10 text-white group-hover:bg-white/20'
                  }`}
                >
